#!/usr/bin/env python3
"""
Real AI Factory Dashboard - Connected to actual backend API
No mock data - everything comes from the API service on port 8080
"""
...
<html>
<head>
    <title>AI Factory Control Center - Real API</title>
    <style>
        body { 
...
<body>
    ...
    <script>
        ...
        const API_BASE = window.location.hostname === 'localhost' ? 'http://localhost:8080' : 'http://' + window.location.hostname + ':8080';
...
async def main():
    ...
    logger.info("Starting Real AI Factory Dashboard on http://localhost:3000")
    logger.info("Connected to API at http://localhost:8080")
    await site.start()
